...
class PipelineTaskManager:
    ...
    async def get_events(self, file_id: str):
        """Async generator yielding SSE-formatted strings for the SSE endpoint.
        ...
        position = 0
        while True:
            # Yield all currently buffered events
            while position < len(state.buffered_events):
                yield state.buffered_events[position]
                position += 1

            # If pipeline is done, stop
            if state.done.is_set():
                return

            # Wait for new events
            state.new_event.clear()
            # Double-check after clear: an event may have been appended
            # between our len() check and clear(), which would be missed
    ...
    async def cleanup(self, file_id: str) -> None:
    ...
    async def _run_nonpdf_pipeline(
        ...
        try:
            ...
            # Read cached text from enqueue_parse, or parse directly
    ...
    @staticmethod
    def _write_cache(owner_id: str, file_id: str, markdown: str) -> None:
        cache_dir = f"/tmp/acacia_uploads/{owner_id}"
...
def _emit(state: _PipelineState, event: str, data: dict) -> None:
